...
void load_map(SDL_Renderer *renderer, string filename, string destWaypointName)
{
  ...
  while (index < strings.size())
  {
    ...
    if (word == "bg" && g_useBackgrounds)
    {
      ...
      background = loadTexture(renderer, "resources/static/backgrounds/" + backgroundstr + "png");
...

  {
    ...
    for (long long unsigned int i = 0; i < g_boxs.size(); i++)
    {
      for (box *box : g_boxs[i])
      {
        ...
        if (box->walltexture == "resources/engine/seethru.png")
        {
          break;
        }
        // handle rect
        ...
        if (box->capped)
        {
          ...
          {
            // shine
            child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", box->bounds.x, box->bounds.y + box->bounds.height + 54 / 2, box->layer * 64 + 64, box->bounds.width, 54);
          ...
          {
            // back
            child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", box->bounds.x, box->bounds.y + 54 / 2, box->layer * 64 + 64, box->bounds.width, 54 / 2);
            child->parent = box;
            box->children.push_back(child);
          }
        }
        ...
        if (box->shadeBot == 1)
        {
          child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", box->bounds.x, box->bounds.y + box->bounds.height + 19 + 2, 64 * box->layer + 2, box->bounds.width, 55);
          child->parent = box;
          box->children.push_back(child);
        }
        // left
        ...
        if (box->shadeLeft)
        {
          for (int i = 0; i < box->bounds.height; i += step)
          {
            child = new mapObject(renderer, "resources/engine/h-OCCLUSION.png", "&", box->bounds.x - 27, box->bounds.y + i + g_platformResolution, 64 * box->layer, 55 / 2, step);
            child->parent = box;
            box->children.push_back(child);
          }
        }
        if (box->shadeRight)
        {
          for (int i = 0; i < box->bounds.height; i += step)
          { // 5, 8
            child = new mapObject(renderer, "resources/engine/h-OCCLUSION.png", "&", box->bounds.x + box->bounds.width, box->bounds.y + i + g_platformResolution, 64 * box->layer, 55 / 2, step);
            child->parent = box;
            box->children.push_back(child);
          }
        }
        ...
        {
          child = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", box->bounds.x - (38 - 19), box->bounds.y, 64 * box->layer, 32, 19, 0, 0);
          child->parent = box;
          box->children.push_back(child);
        }
        // corner b
        ...
        {
          child = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", box->bounds.x + box->bounds.width, box->bounds.y, 64 * box->layer, 32, 19, 0, 0);
          child->parent = box;
          box->children.push_back(child);
        }
        // corner c
        ...
        {
          child = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", box->bounds.x - (38 - 19), box->bounds.y + box->bounds.height + (38 - 19), 64 * box->layer, 19, 19, 0, 0);
          child->parent = box;
          box->children.push_back(child);
        }
        // corner d
        ...
        {
          child = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", box->bounds.x + box->bounds.width, box->bounds.y + box->bounds.height + (38 - 19), 64 * box->layer, 19, 19, 0, 0);
          child->parent = box;
          box->children.push_back(child);
        }
      }
    ...
    for (auto i : g_impliedSlopes) {
      ...
      child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", i->bounds.x, i->bounds.y + 19 + 2, 64 * i->layer + 2, i->bounds.width, 55);
      child->parent = i;
      ...
      if(i->shadeLeft) {
        ...
        {
          child = new mapObject(renderer, "resources/engine/h-OCCLUSION.png", "&", i->bounds.x - 27, i->bounds.y + j + g_platformResolution, 64 * i->layer, 55 / 2, g_platformResolution);
          child->parent = i;
          i->children.push_back(child);
        }

        child = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", i->bounds.x - (38 - 19), i->bounds.y, 64 * i->layer, 19, 19, 0, 0);
        child->parent = i;
        i->children.push_back(child);
      ...
      if(i->shadeRight) {
        for (int j = 0; j < i->bounds.height; j += g_platformResolution)
        { // 5, 8
          child = new mapObject(renderer, "resources/engine/h-OCCLUSION.png", "&", i->bounds.x + i->bounds.width, i->bounds.y + j + g_platformResolution, 64 * i->layer, 55 / 2, g_platformResolution);
          child->parent = i;
          i->children.push_back(child);
        }

        child = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", i->bounds.x + i->bounds.width, i->bounds.y, 64 * i->layer, 32, 19, 0, 0);
        child->parent = i;
        i->children.push_back(child);
      ...
      switch(i->style) {
        case 0: {
          if(i->type == 1) {
            ...
            {
              child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", i->x2 + j, i->y1 + 30 - 64+ (j * XtoY) - 1, i->layer * 64, step, 50, 0, -(j * XtoY));
              child->parent = i;
              i->children.push_back(child);
            }
            ...
            for (int j = 0; j < 64; j += step)
            {
              child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", i->x2 + j - 64, i->y1 + 30 - (j * XtoY) - 1, i->layer * 64, step, 50, 0, (j * XtoY));
              child->parent = i;
              i->children.push_back(child);
            }
        ...
        case 2: { //incurve
          if(i->type == 1) {
            ...
            for (int j = 0; j < 64; j += step)
            {
              child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", i->x2 - j + 64, i->y1 + 55 + 30 - ((64 - pow(pow(64, 2) - pow(j, 2), 0.5)) * XtoY) - 1 - 64, i->layer * 64, step, 50, 0, ((64 - pow(pow(64, 2) - pow(j, 2), 0.5)) * XtoY) + 0);
              child->parent = i;
              i->children.push_back(child);
            }
        ...
        if (triangle->style == 0)
        {
          if (triangle->type == 0)
          {
            ...
            if (triangle->capped)
            {
              ...
              {
                child = new mapObject(renderer, triangle->captexture, "resources/engine/a.png", triangle->x2, triangle->y1 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
              ...
              {
                child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i, triangle->y1 + 55 + 35 - (i * XtoY) - 1, triangle->layer * 64 + 64, step, 55, 0, (i * XtoY) + 0);
            ...
            if (triangle->shaded)
            {
              for (int i = 0; i < 64; i += step)
              {
                ...
                child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", triangle->x2 + i, triangle->y1 + 55 + 30 - (i * XtoY) - 1, triangle->layer * 64, step, 50, 0, (i * XtoY) + 0);
                child->parent = triangle;
                triangle->children.push_back(child);
              }
            }
          ...
          else
          {
            if (triangle->type == 3)
            {
              ...
              if (triangle->capped)
              {
                for (int i = 0; i < 55; i += step)
                {
                  child = new mapObject(renderer, triangle->captexture, "resources/engine/b.png", triangle->x1 + 1, triangle->y1 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                  child->parent = triangle;
                  triangle->children.push_back(child);
                }
                step = g_TiltResolution;
                for (int i = 0; i < 64; i += step)
                {
                  child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i - 64, triangle->y1 + 55 + 35 - (((64 - step) - i) * XtoY) - 1, triangle->layer * 64 + 64, step, 55, 0, ((64 - i) * XtoY));
                  child->sortingOffset = -25;
                  child->parent = triangle;
                  triangle->children.push_back(child);
                }
              ...
              if (triangle->shaded)
              {
                for (int i = 0; i < 64; i += step)
                {
                  ...
                  child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", triangle->x2 + i - 64, triangle->y1 + 55 + 30 - (((64 - step) - i) * XtoY) - 1, 64 * triangle->layer, step, 50, 0, ((64 - i) * XtoY));
                  child->parent = triangle;
                  triangle->children.push_back(child);
                }
              }
            ...
            else
            {
              if (triangle->type == 2)
              {
                if (triangle->capped)
                {
                  ...
                  for (int i = 0; i < 55; i += step)
                  {
                    child = new mapObject(renderer, triangle->captexture, "resources/engine/c.png", triangle->x1 + 1, triangle->y2 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                  step = g_TiltResolution;
                  for (int i = 0; i < 64; i += step)
                  {
                    child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i - 64, triangle->y1 + 35 - (i * XtoY) - 1, triangle->layer * 64 + 64, step, 34, 0, (i * XtoY) + 0);
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                }
                // child = new mapObject(renderer, triangle->captexture, "resources/engine/c.png", triangle->x1 + 1, triangle->y2 + 55 + 1, triangle->layer * 64 + 64, 64 + 1, 54 + 1, 0, 0, 0);
                // triangle->children.push_back(child);
              }
              else
              {
                ...
                if (triangle->capped)
                {
                  ...
                  for (int i = 0; i < 55; i += step)
                  {
                    child = new mapObject(renderer, triangle->captexture, "resources/engine/d.png", triangle->x2, triangle->y2 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                  step = g_TiltResolution;
                  for (int i = 0; i < 64; i += step)
                  {
                    child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i, triangle->y1 + 35 - (((64 - step) - i) * XtoY) - 1, triangle->layer * 64 + 64, step, 34, 0, ((64 - i) * XtoY));
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                }
        ...
        else if (triangle->style == 1)
        {
          if (triangle->type == 0)
          {
            ...
            if (triangle->capped)
            {
              for (int i = 0; i < 55; i += step)
              {
                child = new mapObject(renderer, triangle->captexture, "resources/engine/aro.png", triangle->x2, triangle->y1 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                child->parent = triangle;
                triangle->children.push_back(child);
              }
              // diagonal shine
              ...
              for (int i = 0; i < 64; i += step)
              {
                child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i, triangle->y1 + 55 + 35 - ((64 - pow(pow(64, 2) - pow(i, 2), 0.5)) * XtoY) - 1, triangle->layer * 64 + 64, step, 55, 0, ((64 - pow(pow(64, 2) - pow(i, 2), 0.5)) * XtoY) + 0);
            ...
            if (triangle->shaded)
            {
              for (int i = 0; i < 64; i += step)
              {
                ...
                child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", triangle->x2 + i, triangle->y1 + 55 + 30 - ((64 - pow(pow(64, 2) - pow(i, 2), 0.5)) * XtoY) - 1, triangle->layer * 64, step, 50, 0, ((64 - pow(pow(64, 2) - pow(i, 2), 0.5)) * XtoY) + 0);
                child->parent = triangle;
                triangle->children.push_back(child);
              }

              //This is to fill in a "gap" of shading between this corner and the block behind
              child = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", triangle->x2 + 64, triangle->y1 + 19, 64 * triangle->layer, 19, 19, 0, 0);
              child->parent = triangle;
              triangle->children.push_back(child);
            }
          }
          else
          {
            if (triangle->type == 3)
            {
              ...
              if (triangle->capped)
              {
                for (int i = 0; i < 55; i += step)
                {
                  child = new mapObject(renderer, triangle->captexture, "resources/engine/bro.png", triangle->x1 + 1, triangle->y1 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                  child->parent = triangle;
                  triangle->children.push_back(child);
                }
                step = g_TiltResolution;
                for (int i = 0; i < 64; i += step)
                {
                  child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i - 64, triangle->y1 + 55 + 35 - (((64 - pow(pow(64, 2) - pow(64 - i, 2), 0.5))) * XtoY) - 1, triangle->layer * 64 + 64, step, 55, 0, (((64 - pow(pow(64, 2) - pow(64 - i, 2), 0.5))) * XtoY));
                  child->sortingOffset = -15;
                  child->parent = triangle;
                  triangle->children.push_back(child);
                }
              ...
              if (triangle->shaded)
              {
                for (int i = 0; i < 64; i += step)
                {
                  ...
                  child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", triangle->x2 + i - 64, triangle->y1 + 55 + 30 - (((64 - pow(pow(64, 2) - pow(64 - i, 2), 0.5))) * XtoY), 64 * triangle->layer, step, 50, 0, (((64 - pow(pow(64, 2) - pow(64 - i, 2), 0.5))) * XtoY));
                  child->parent = triangle;
                  triangle->children.push_back(child);
                }
                //This is to fill in a "gap" of shading between this corner and the block behind
                child = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", triangle->x1 - 19, triangle->y1 + 19, 64 * triangle->layer, 19, 19, 0, 0);
                child->parent = triangle;
                triangle->children.push_back(child);
              }
            }
            else
            {
              if (triangle->type == 2)
              {
                if (triangle->capped)
                {
                  ...
                  for (int i = 0; i < 55; i += step)
                  {
                    child = new mapObject(renderer, triangle->captexture, "resources/engine/cro.png", triangle->x1 + 1, triangle->y2 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                  step = g_TiltResolution;
                  for (int i = 0; i < 64; i += step)
                  {
                    child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i - 64, triangle->y1 + 35 - (((pow(pow(64, 2) - pow(64 - i, 2), 0.5))) * XtoY) - 1, triangle->layer * 64 + 64, step, 34, 0, (((pow(pow(64, 2) - pow(64 - i, 2), 0.5))) * XtoY) + 0);
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                }
              ...
              else
              {
                ...
                if (triangle->capped)
                {
                  ...
                  for (int i = 0; i < 55; i += step)
                  {
                    child = new mapObject(renderer, triangle->captexture, "resources/engine/dro.png", triangle->x2, triangle->y2 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                  step = g_TiltResolution;
                  for (int i = 0; i < 64; i += step)
                  {
                    child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i, triangle->y1 + 35 - (((pow(pow(64, 2) - pow(i, 2), 0.5))) * XtoY) - 0.5, triangle->layer * 64 + 64, step, 34, 0, (((pow(pow(64, 2) - pow(i, 2), 0.5))) * XtoY));
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                }
        ...
        else if (triangle->style == 2)
        {
          if (triangle->type == 0)
          {
            ...
            if (triangle->capped)
            {
              for (int i = 0; i < 55; i += step)
              {
                child = new mapObject(renderer, triangle->captexture, "resources/engine/ari.png", triangle->x2, triangle->y1 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                child->parent = triangle;
                triangle->children.push_back(child);
              }
              // diagonal shine
              ...
              for (int i = 0; i < 64; i += step)
              {
                child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i, triangle->y1 + 35 + ((64 - pow(pow(64, 2) - pow(64 - i, 2), 0.5)) * XtoY) - 1, triangle->layer * 64 + 64, step, 55, 0, -((64 - pow(pow(64, 2) - pow(64 - i, 2), 0.5)) * XtoY) + 0);
                child->sortingOffset = -25;
                child->parent = triangle;
                triangle->children.push_back(child);
              }
            ...
            if (triangle->shaded)
            {
              for (int i = 0; i < 64; i += step)
              {
                ...
                child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", triangle->x2 + i, triangle->y1 + 30 + ((64 - pow(pow(64, 2) - pow(64 - i, 2), 0.5)) * XtoY) - 1, triangle->layer * 64, step, 50, 0, -((64 - pow(pow(64, 2) - pow(64 - i, 2), 0.5)) * XtoY) + 0);
                child->parent = triangle;
                triangle->children.push_back(child);
              ...
              //This is to fill in a "gap" of shading between this corner and the block behind
              child = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", triangle->x2, triangle->y1 + 55, 64 * triangle->layer, 19, 19, 0, 0);
              child->parent = triangle;
              triangle->children.push_back(child);
            }
          }
          else
          {
            if (triangle->type == 3)
            {
              ...
              if (triangle->capped)
              {
                for (int i = 0; i < 55; i += step)
                {
                  child = new mapObject(renderer, triangle->captexture, "resources/engine/bri.png", triangle->x1 + 1, triangle->y1 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                  child->parent = triangle;
                  triangle->children.push_back(child);
                }
                step = g_TiltResolution;
                for (int i = 0; i < 64; i += step)
                {
                  child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i - 64, triangle->y1 + 55 + 35 - (((64 - step) - (64 - pow(pow(64, 2) - pow(i, 2), 0.5))) * XtoY) - 1, triangle->layer * 64 + 64, step, 55, 0, -(64 - pow(pow(64, 2) - pow(i, 2), 0.5)));
                  child->sortingOffset = -25;
                  child->parent = triangle;
                  triangle->children.push_back(child);
                }
              ...
              if (triangle->shaded)
              {
                for (int i = 0; i < 64; i += step)
                {
                  ...
                  child = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", triangle->x2 + i - 64, triangle->y1 + 55 + 30 - (((64 - step) - (64 - pow(pow(64, 2) - pow(i, 2), 0.5))) * XtoY) - 1, 64 * triangle->layer, step, 50, 0, (((64 - step) - (64 - pow(pow(64, 2) - pow(i, 2), 0.5))) * XtoY));
                  child->parent = triangle;
                  triangle->children.push_back(child);
                }

                //This is to fill in a "gap" of shading between this corner and the block behind
                child = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", triangle->x2 - 19, triangle->y1 + 55, 64 * triangle->layer, 19, 19, 0, 0);
                child->parent = triangle;
                triangle->children.push_back(child);
              }
            }
            else
            {
              if (triangle->type == 2)
              {
                if (triangle->capped)
                {
                  ...
                  for (int i = 0; i < 55; i += step)
                  {
                    child = new mapObject(renderer, triangle->captexture, "resources/engine/cri.png", triangle->x1 + 1, triangle->y2 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                  step = g_TiltResolution;
                  for (int i = 0; i < 64; i += step)
                  {
                    child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i - 64, triangle->y1 + 35 - ((64 - pow(pow(64, 2) - pow(i, 2), 0.5)) * XtoY) - 1, triangle->layer * 64 + 64, step, 34, 0, ((64 - pow(pow(64, 2) - pow(i, 2), 0.5)) * XtoY));
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                }
              ...
              else
              {
                ...
                if (triangle->capped)
                {
                  ...
                  for (int i = 0; i < 55; i += step)
                  {
                    child = new mapObject(renderer, triangle->captexture, "resources/engine/dri.png", triangle->x2, triangle->y2 + i + step, triangle->layer * 64 + 64, 64 - 1, step, 0);
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                  step = g_TiltResolution;
                  for (int i = 0; i < 64; i += step)
                  {
                    child = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png", "&", triangle->x2 + i, triangle->y1 + 35 - ((64 - pow(pow(64, 2) - pow(64 - i, 2), 0.5)) * XtoY) - 1, triangle->layer * 64 + 64, step, 34, 0, ((64 - pow(pow(64, 2) - pow(64 - i, 2), 0.5)) * XtoY));
                    child->parent = triangle;
                    triangle->children.push_back(child);
                  }
                }
...
void changeTheme(string str)
{
  ...
  // }
  // SDL_Surface* bs = IMG_Load(("resources/static/backgrounds/" + backgroundstr + ".png").c_str());
...
//  }
  ...
  for (long long unsigned int i = 0; i < g_tiles.size(); i++)
  {
    ...
    if (g_tiles[i]->fileaddress.find("resources/engine") != string::npos && g_tiles[i]->fileaddress != "resources/engine/black-diffuse.png")
    {
      continue;
    }
...
{
  selection = new tile(renderer, "resources/engine/marker.png", "&", 0, 0, 0, 0, 1, 1, 1, 0, 0);
  marker = new tile(renderer, "resources/engine/marker.png", "&", 0, 0, 0, 0, 2, 0, 0, 0, 0);
  markerz = new tile(renderer, "resources/engine/marker.png", "&", 0, 0, 0, 0, 2, 0, 0, 0, 0);
  worldsoundIcon = new tile(renderer, "resources/engine/speaker.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  listenerIcon = new tile(renderer, "resources/engine/listener.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  navNodeIconBlue = new tile(renderer, "resources/engine/walkerBlue.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  navNodeIconRed = new tile(renderer, "resources/engine/walkerRed.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  navNodeIconYellow = new tile(renderer, "resources/engine/walkerYellow.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  musicIcon = new tile(renderer, "resources/engine/music.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  cueIcon = new tile(renderer, "resources/engine/cue.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  waypointIcon = new tile(renderer, "resources/engine/waypoint.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  poiIcon = new tile(renderer, "resources/engine/poi.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  doorIcon = new tile(renderer, "resources/engine/door.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  ddoorIcon = new tile(renderer, "resources/engine/ddoor.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
  triggerIcon = new tile(renderer, "resources/engine/trigger.png", "&", 0, 0, 0, 0, 1, 0, 0, 0, 0);
...
//         SDL_RenderCopy(renderer, navNodeIconBlue->texture, NULL, &obj);
  ...
  if (devinput[0] && !olddevinput[0] && !makingbox)
  {
    ...
    ly = py;
    makingbox = 1;
    selection->texture = loadTexture(renderer, "resources/engine/trigger.png");
  ...
  if (devinput[3] && !olddevinput[3] && makingbox == 0)
  {
    ...
    if (nudge == 0)
    {
      ...
      ly = py;
      makingbox = 1;
      selection->texture = loadTexture(renderer, "resources/engine/wall.png");
    }
  }
  else
  {
    if (devinput[3] && !olddevinput[3] && makingbox == 1)
    {
      if (makingbox)
      {
        ...
        // string loadstr = "resources/static/wall.png";
        if (makeboxs)
        {
        ...
        {
          // front
          // mapObject* f = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png",  "&", selection->x, selection->y + selection->height + marker->height/2,  wallheight + 1, selection->width, marker->height);

          // back
          // f = new mapObject(renderer, "resources/engine/SMOOTHSHADING.png",  "&", selection->x, selection->y + marker->height/2, wallheight + 1, selection->width, marker->height/2);
        }

        if (shine == 2)
        {
          mapObject *f = new mapObject(renderer, "resources/engine/SHARPSHADING.png", "&", selection->x, selection->y + selection->height - wallheight + marker->height / 2, 0, selection->width, marker->height);
          (void)f;
        }

        if (shine == 3)
        {
          mapObject *f = new mapObject(renderer, "resources/engine/SHARPBRIGHTSHADING.png", "&", selection->x, selection->y + selection->height - wallheight + marker->height / 2, 0, selection->width, marker->height);
          (void)f;
        }
        ...
        {
          // front shading
          mapObject *e = new mapObject(renderer, "resources/engine/OCCLUSION.png", "&", selection->x, selection->y + selection->height + 19, 0, selection->width, marker->height);
          c->children.push_back(e);
          // left
          int step = g_platformResolution;
          for (int i = 0; i < selection->height; i += step)
          {
            mapObject *e = new mapObject(renderer, "resources/engine/h-OCCLUSION.png", "&", selection->x - 19, selection->y + i + g_platformResolution, 0, 55 / 2, step);
            c->children.push_back(e);
          }
          for (int i = 0; i < selection->height; i += step)
          {
            mapObject *e = new mapObject(renderer, "resources/engine/h-OCCLUSION.png", "&", selection->x + selection->width, selection->y + i + g_platformResolution, 0, 55 / 2, step);
            c->children.push_back(e);
          }

          // corner a
          e = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", selection->x - (38 - 19), selection->y, 0, 32, 19, 0, 0);
          c->children.push_back(e);
          // corner b
          e = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", selection->x + selection->width, selection->y, 0, 32, 19, 0, 0);
          c->children.push_back(e);
          // corner c
          e = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", selection->x - (38 - 19), selection->y + selection->height + (38 - 19), 0, 19, 19, 0, 0);
          c->children.push_back(e);
          // corner d
          e = new mapObject(renderer, "resources/engine/x-OCCLUSION.png", "&", selection->x + selection->width, selection->y + selection->height + (38 - 19), 0, 19, 19, 0, 0);
          c->children.push_back(e);
        }
      }
    }
  ...
  if (devinput[6] && !olddevinput[6] && makingbox == 0)
  {
    ...
    ly = py;
    makingbox = 1;
    selection->texture = loadTexture(renderer, "resources/engine/invisiblewall.png");
  }
  else
  {
  ...
  if (devinput[34] && !olddevinput[34] && makingbox == 0)
  {
    ...
    ly = py;
    makingbox = 1;
    selection->image = IMG_Load("resources/engine/invisiblewall.png");
  ...
  if (devinput[20] && !olddevinput[20] && makingbox == 0)
  {
    ...
    ly = py;
    makingbox = 1;
    selection->image = IMG_Load("resources/engine/navmesh.png");
    selection->texture = SDL_CreateTextureFromSurface(renderer, selection->image);
    SDL_FreeSurface(selection->image);
  }
  else
  ...
  if (devinput[5] && !olddevinput[5] && makingdoor == 0)
  {
    ...
    ly = py;
    makingdoor = 1;
    selection->image = IMG_Load("resources/engine/door.png");
    selection->wraptexture = 0;
    selection->texture = SDL_CreateTextureFromSurface(renderer, selection->image);
    SDL_FreeSurface(selection->image);
  }
  ...
  if (devinput[9] && !olddevinput[9] && makingdoor == 0)
  {
    ...
    ly = py;
    makingdoor = 1;
    selection->image = IMG_Load("resources/engine/ddoor.png");
    selection->wraptexture = 0;
    selection->texture = SDL_CreateTextureFromSurface(renderer, selection->image);
    SDL_FreeSurface(selection->image);
  }
      ...
      if (word == "easybake" || word == "eb")
      {
        ...
        for (long long unsigned int i = 0; i < g_boxs.size(); i++)
        {
          for (auto b : g_boxs[i])
          {
            ...
            {
              // don't calculate lighting by invisible walls
              if (n->walltexture == "resources/engine/seethru.png")
              {
                continue;
              }
        ...
        for (long long unsigned int i = 0; i < g_boxs.size(); i++)
        {
          ...
          for (auto b : g_boxs[i])
          {
            ...
            if (b->walltexture == "resources/engine/seethru.png")
            {
              b->shineTop = 0;
              b->shineBot = 0;
              continue;
...
//        }
      ...
      if (word == "set" || word == "s")
      {
        ...
        {
          line >> walltex;
          walltex = "resources/static/diffuse/" + textureDirectory + walltex + ".png";
        ...
        if (word == "floor" || word == "f")
        {
          ...
          if (floortex == "black")
          {
            floortex = "resources/engine/black-diffuse.png";
          }
          else
          {
            floortex = "resources/static/diffuse/" + textureDirectory + floortex + ".png";
          }
          break;
        }
        if (word == "cap" || word == "c")
        {
          line >> captex;
          captex = "resources/static/diffuse/" + textureDirectory + captex + ".png";
          break;
        }
        if (word == "mask" || word == "m")
        {
          line >> masktex;
          masktex = "resources/static/masks/" + masktex + ".png";
          break;
        }
        if (word == "layer")
        {
      ...
      if (word == "reset" || word == "rs")
      {
        ...
        if (word == "texturedirectory" || word == "td" || word == "theme")
        {
          ...
          captex = "resources/static/diffuse/mapeditor/a.png";
          walltex = "resources/static/diffuse/mapeditor/c.png";
          floortex = "resources/static/diffuse/mapeditor/b.png";
          break;
        }
        if (word == "grid")
        {
  ...
  if (devinput[12] && !olddevinput[12])
  {
    ...
    if (autoMakeWallcaps)
    {
      ...
      for (int i = 0; i < 55; i += step)
      {
        mapObject *e = new mapObject(renderer, captex, "resources/engine/a.png", marker->x, marker->y + i + step, wallheight, 64 - 1, step, 0);
    ...
    if (autoMakeWalls)
    {
      ...
      // tile* t = new tile(renderer, walltex.c_str(), "resources/engine/a.png", marker->x, marker->y - 1, 64 - 1, 54 + 1, layer, 1, 1, 0, 0);
  ...
  if (devinput[13] && !olddevinput[13])
  {
    ...
    if (autoMakeWallcaps)
    {
      ...
      for (int i = 0; i < 55; i += step)
      {
        mapObject *e = new mapObject(renderer, captex, "resources/engine/b.png", marker->x + 1, marker->y + i + step, wallheight, 64 - 1, step, 0);
        n->children.push_back(e);
      }
    }

    // a tile on the floor to help with the edge of the diagonal wall pieces
    // tile* t = new tile(renderer, walltex.c_str(), "resources/engine/b.png", marker->x + 1, marker->y - 1, 64 - 1, 54 + 1, layer, 1, 1, 0, 0);
  ...
  if (devinput[14] && !olddevinput[14])
  {
    ...
    for (int i = 0; i < 55; i += step)
    {
      mapObject *child = new mapObject(renderer, n->captexture, "resources/engine/c.png", n->x1, n->y2 + i + step, n->layer * 64 + 64, 64 - 1, step, 0);
  ...
  if (devinput[15] && !olddevinput[15])
  {
    ...
    for (int i = 0; i < 55; i += step)
    {
      mapObject *child = new mapObject(renderer, n->captexture, "resources/engine/d.png", n->x2, n->y2 + i + step, n->layer * 64 + 64, 64 - 1, step, 0);
      n->children.push_back(child);
    }
  }
  ...
  if (devinput[26] && !olddevinput[26])
  {
    ...
    if (autoMakeWallcaps)
    {
      ...
      for (int i = 0; i < 55; i += step)
      {
        mapObject *e = new mapObject(renderer, captex, "resources/engine/aro.png", marker->x, marker->y + i + step, wallheight, 64 - 1, step, 0);
        n->children.push_back(e);
      }
    }
  ...
  if (devinput[27] && !olddevinput[27])
  {
    ...
    if (autoMakeWallcaps)
    {
      ...
      for (int i = 0; i < 55; i += step)
      {
        mapObject *e = new mapObject(renderer, captex, "resources/engine/bro.png", marker->x + 1, marker->y + i + step, wallheight, 64 - 1, step, 0);
        n->children.push_back(e);
      }
    }
  ...
  if (devinput[28] && !olddevinput[28])
  {
    ...
    for (int i = 0; i < 55; i += step)
    {
      mapObject *child = new mapObject(renderer, n->captexture, "resources/engine/cro.png", n->x1, n->y2 + i + step, n->layer * 64 + 64, 64 - 1, step, 0);
      n->children.push_back(child);
    }
  }
  if (devinput[29] && !olddevinput[29])
  {
    ...
    for (int i = 0; i < 55; i += step)
    {
      mapObject *child = new mapObject(renderer, n->captexture, "resources/engine/dro.png", n->x2, n->y2 + i + step, n->layer * 64 + 64, 64 - 1, step, 0);
      n->children.push_back(child);
    }
  }
  ...
  if (devinput[30] && !olddevinput[30])
  {
    ...
    if (autoMakeWallcaps)
    {
      ...
      for (int i = 0; i < 55; i += step)
      {
        mapObject *e = new mapObject(renderer, captex, "resources/engine/ari.png", marker->x, marker->y + i + step, wallheight, 64 - 1, step, 0);
        n->children.push_back(e);
      }
    }
  ...
  if (devinput[31] && !olddevinput[31])
  {
    ...
    if (autoMakeWallcaps)
    {
      ...
      for (int i = 0; i < 55; i += step)
      {
        mapObject *e = new mapObject(renderer, captex, "resources/engine/bri.png", marker->x + 1, marker->y + i + step, wallheight, 64 - 1, step, 0);
        n->children.push_back(e);
      }
    }
  ...
  if (devinput[32] && !olddevinput[32])
  {
    ...
    for (int i = 0; i < 55; i += step)
    {
      mapObject *child = new mapObject(renderer, n->captexture, "resources/engine/cri.png", n->x1, n->y2 + i + step, n->layer * 64 + 64, 64 - 1, step, 0);
      n->children.push_back(child);
    }
  }
  ...
  if (devinput[33] && !olddevinput[33])
  {
    ...
    for (int i = 0; i < 55; i += step)
    {
      mapObject *child = new mapObject(renderer, n->captexture, "resources/engine/dri.png", n->x2, n->y2 + i + step, n->layer * 64 + 64, 64 - 1, step, 0);
      n->children.push_back(child);
    }
  }
  ...
  if (devinput[19] && !olddevinput[19] && makingbox == 0)
  {
    ...
    ly = py;
    makingbox = 1;
    selection->image = IMG_Load("resources/engine/collisionzone.png");
    selection->texture = SDL_CreateTextureFromSurface(renderer, selection->image);
    SDL_FreeSurface(selection->image);
  }
  else

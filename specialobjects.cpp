    ...
    case 34:
    {
      ...
      if(g_tallGrasses.size() > 0) {
        ...
        if(ag.size() > 0) {
          ...
          a->setOriginX(newX);
          a->setOriginY(newY);
          a->level = 1; //this is used to stop chasing the player when it reaches them
          ag[rand]->enabled = 0;
          g_worldEnemies.push_back(a);
          //M("pushed someone back to g_worldEnemies");
      ...
      if(RectOverlap(protag->getMovedBounds(), a->getMovedBounds()) && a->opacity_delta >= 0 && devMode == 0) {
        ...
        if(g_catchUpMode == 0) {
          ...
          for(auto x : g_worldEnemies) {
            rect obj;
            obj = rect(
                ((x->x) -g_camera.x + (x->width-(x->curwidth))/2)* 1 ,
                ((x->y) - (((x->curheight) * (XtoY) + (x->height * (1-XtoY)))) - ((x->z) + x->floatheight) * XtoZ) - g_camera.y,
                (x->curwidth),
                (x->curheight)
                );
  //          obj = rect(
  //            x->x - g_camera.x,
  //            x->y - g_camera.y  - (x->z * XtoZ),
  //            x->width,
  //            x->height
  //            );
...
//            D(cam.height);
        ...
        //because when we're in catchupmode, entities
        //are using pathfinding to get to fomm
//        if(a->level!=0) {
//          a->level = 0;
//          a->semisolid = 0; 
//          M("Set level to 0");
